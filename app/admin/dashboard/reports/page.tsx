...
export default function AdminReportsPage() {
  ...
  const filteredReports = reports.filter(
    (report) =>
      ...
      report.agencyName.toLowerCase().includes(searchQuery.toLowerCase()),
  )
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Consultez les rapports soumis par les agences</p>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Aucun rapport trouvé</h3>
            <p className="text-gray-600 max-w-md mb-6">Les rapports soumis par les agences s'afficheront ici.</p>
          </div>
        ) : (
          <motion.div
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredReports.map((report) => (
                    <tr key={report.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-sm text-gray-600">{report.agencyName}</td>
                      <td className="py-3 px-4 text-sm text-gray-600">{formatDate(report.submittedAt)}</td>
