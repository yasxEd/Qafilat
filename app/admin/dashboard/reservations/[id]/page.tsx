...
export default function ReservationDetailsPage() {
  ...
  const handlePreview = (docId: string) => {
    if (!reservation?.documents[docId as keyof typeof reservation.documents]?.exists) return

    const docInfo = requiredDocuments.find((doc) => doc.id === docId)
    if (!docInfo) return

    const document = reservation.documents[docId as keyof typeof reservation.documents]
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-900">{reservation.medicalConditions}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {requiredDocuments.map((doc) => {
                  const docData = reservation.documents?.[doc.id]
